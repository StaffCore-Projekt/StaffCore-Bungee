    ...
    public void init() {
        ...
        try {
            ...
            Main.getInstance().getProxy().getConsole().sendMessage(
                new TextComponent("§cSystem §8» §aSuccessfully §8cached §7" + fallback.size() + " §aFallback-Translations§8(§7US§8)"));
            Main.getInstance().getProxy().getConsole().sendMessage(new TextComponent(""));
        } catch (NullPointerException e) {
            e.printStackTrace();
            Main.getInstance().getProxy().getConsole().sendMessage(new TextComponent(""));
    ...
    public void fetch(String lang) {
        if (availableLanguages.containsKey(lang)) {
            ...
            try {
                ...
                Main.getInstance().getProxy().getConsole().sendMessage(new TextComponent(""));
                Main.getInstance().getProxy().getConsole().sendMessage(new TextComponent(
                    "§cSystem §8» §aSuccessfully §8cached §7" + language.size() + " §aRequest-Translations§8(§7" + lang.toUpperCase() +
                        "§8)"));
                Main.getInstance().getProxy().getConsole().sendMessage(new TextComponent(""));
            } catch (NullPointerException e) {
                Main.getInstance().getProxy().getConsole().sendMessage(new TextComponent(""));
            ...
            for (String key : availableLanguages.keySet()) {
                Main.getInstance().getProxy().getConsole()
                    .sendMessage(new TextComponent("§8- §c" + key + "§8 | §c" + availableLanguages.get(key)));
            }
        }
    }
    ...
    public void fetchCustom(String restapikey) {
        ...
        try {
            JsonObject translation =
                readJsonFromUrl("https://www.lacodev.de/services/api/v1/restricted/key/" + restapikey).getAsJsonObject();
            ...
            Main.getInstance().getProxy().getConsole().sendMessage(new TextComponent(""));
            Main.getInstance().getProxy().getConsole()
                .sendMessage(new TextComponent("§cSystem §8» §aSuccessfully §8cached §7" + custom.size() + " §aMessages§8(§7CUSTOM§8)"));
            Main.getInstance().getProxy().getConsole().sendMessage(new TextComponent(""));
        } catch (ClassCastException | NullPointerException e) {
            Main.getInstance().getProxy().getConsole().sendMessage(new TextComponent(""));
            Main.getInstance().getProxy().getConsole().sendMessage(new TextComponent("§cSystem §8» §7Translator denied your request!"));
            Main.getInstance().getProxy().getConsole()
                .sendMessage(new TextComponent("§cSystem §8» §cServices might be in maintenance! Or you entered the wrong API-Key"));
            Main.getInstance().getProxy().getConsole()
                .sendMessage(new TextComponent("§cSystem §8» §cTry adding messages ;) If you have, please contact our support!"));
            Main.getInstance().getProxy().getConsole().sendMessage(new TextComponent(""));
        }
