    ...
    public void execute(CommandSender sender, String[] args) {
        ...
        if (sender instanceof ProxiedPlayer) {
            ...
            if (player.hasPermission(Main.getPermissionNotice("Permissions.Everything")) ||
                player.hasPermission(Main.getPermissionNotice("Permissions.StaffRollback.Use"))) {
                        ...
                        ProxyServer.getInstance().getScheduler().runAsync(Main.getInstance(), new Runnable() {
                            ...
                            public void run() {
                                ...
                                Main.getMySQL().update(
                                    "INSERT INTO StaffCore_activitydb(type,uuid,target,message,reg_date,priority) VALUES ('STAFF_ROLLEDBACK','" +
                                        player.getUniqueId().toString() + "',"
                                        + "'" + PlayerManager.getUsernamebyUUID(uuid) +
                                        "','%player% rolled back all actions performed by %target%','" + System.currentTimeMillis() +
                                        "','3')");

                                player.sendMessage(new TextComponent(
                                    Main.getPrefix() + Main.getMSG("Messages.System.Staff-Rollback.Success").replace("%staff%", args[0])));
                ...
                player.sendMessage(new TextComponent(Main.getPrefix() + Main.getMSG("Messages.System.No-Permission")
                    .replace("%permission%", Main.getPermissionNotice("Permissions.StaffRollback.Use"))));
            }
