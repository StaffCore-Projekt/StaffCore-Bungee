    ...
    public void execute(CommandSender sender, String[] args) {
        ...
        if (sender instanceof ProxiedPlayer) {
            ...
            if (player.hasPermission(Main.getPermissionNotice("Permissions.Everything")) ||
                player.hasPermission(Main.getPermissionNotice("Permissions.CommandSpy.Use"))) {
                ...
                if (args.length == 0) {
                    ...
                    if (ProxyServer.getInstance().getServerInfo(args[0]) != null) {
                        ...
                        player.sendMessage(new TextComponent(
                            Main.getPrefix() + Main.getMSG("Messages.CommandSpy.Spying-Specific-Server").replace("%server%", args[0])));
                ...
                player.sendMessage(new TextComponent(Main.getPrefix() + Main.getMSG("Messages.System.No-Permission")
                    .replace("%permission%", Main.getPermissionNotice("Permissions.CommandSpy.Use"))));
            }
