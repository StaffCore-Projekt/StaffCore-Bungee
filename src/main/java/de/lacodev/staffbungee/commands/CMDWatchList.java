        ...
        if (sender instanceof ProxiedPlayer) {
            ...
            if (Main.getMySQL().isConnected()) {
                ...
                if (player.hasPermission(Main.getPermissionNotice("Permissions.Everything")) ||
                    player.hasPermission(Main.getPermissionNotice("Permissions.WatchList.Change"))) {
                        ...
                        if (uuid != null) {
                            ...
                            if (args[0].equalsIgnoreCase("add")) {
                                ...
                                if (!WatchListManager.isWatching(uuid)) {
                                    ...
                                    player.sendMessage(
                                        new TextComponent(Main.getPrefix() + Main.getMSG("Messages.WatchList.Add-Player.Success")));

                                } else {
                                    player.sendMessage(
                                        new TextComponent(Main.getPrefix() + Main.getMSG("Messages.Watchlist.Already-Watching")));
                                }
                                ...
                                if (WatchListManager.isWatching(uuid)) {
                                    ...
                                    player.sendMessage(
                                        new TextComponent(Main.getPrefix() + Main.getMSG("Messages.WatchList.Remove-Player.Success")));

                                } else {
                                    player.sendMessage(
                                        new TextComponent(Main.getPrefix() + Main.getMSG("Messages.Watchlist.Not-Watching")));
                                }
                    ...
                    player.sendMessage(new TextComponent(Main.getPrefix() + Main.getMSG("Messages.System.No-Permission")
                        .replace("%permission%", Main.getPermissionNotice("Permissions.WatchList.Change"))));
                }
