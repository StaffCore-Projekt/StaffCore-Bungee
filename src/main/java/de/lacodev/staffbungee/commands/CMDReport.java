        ...
        if (sender instanceof ProxiedPlayer) {
            ...
            if (args.length == 2) {
                ...
                if (!args[0].equalsIgnoreCase("claim")) {
                        ...
                        if (target != player) {
                            ...
                            try {
                                ...
                                if (ReasonManager.getReasonById(ReasonType.REPORT, id) != null) {
                                    ...
                                    ReportManager.createPlayerReport(ReportType.PLAYER_REPORT, target.getUniqueId().toString(),
                                        player.getUniqueId().toString(), ReasonManager.getReasonById(ReasonType.REPORT, id).getName());

                                    if (!(player.hasPermission(Main.getPermissionNotice("Permissions.Everything")) ||
                                        player.hasPermission(Main.getPermissionNotice("Permissions.Report.Spam.Bypass")))) {
                                        ListenerChat.reportspam.put(player, System.currentTimeMillis() +
                                            (1000 * Integer.valueOf(Settings.REPORT_ANTISPAM_COOLDOWN_SECONDS.getStandard())));
                                    }

                                    Main.getMySQL().update(
                                        "INSERT INTO StaffCore_activitydb(type,uuid,target,message,reg_date,priority) VALUES ('REPORT_CREATED','" +
                                            player.getUniqueId().toString() + "',"
                                            + "'" + target.getUniqueId().toString() + "','%player% reported the player %target%','" +
                                    ...
                                    player.sendMessage(
                                        new TextComponent(Main.getPrefix() + Main.getMSG("Messages.Report-System.No-Reportreasons")));
                                }
                                ...
                                if (ReasonManager.existsReason(ReasonType.REPORT, args[1])) {
                                    ...
                                    ReportManager.createPlayerReport(ReportType.PLAYER_REPORT, target.getUniqueId().toString(),
                                        player.getUniqueId().toString(), args[1]);
                            ...
                            player.sendMessage(
                                new TextComponent(Main.getPrefix() + Main.getMSG("Messages.Report-System.Cannot-report-self")));
                        }
                    ...
                    if (player.hasPermission(Main.getPermissionNotice("Permissions.Everything")) ||
                        player.hasPermission(Main.getPermissionNotice("Permissions.Report.Claim"))) {

                        if (!ReportManager.claimed.containsKey(player.getUniqueId().toString())) {
                            ProxiedPlayer target = ProxyServer.getInstance().getPlayer(UUID.fromString(args[1]));
                            ...
                            if (target != null) {
                                ...
                                if (ReportManager.hasOpenReports(target.getUniqueId().toString())) {
                                    ...
                                    player.sendMessage(new TextComponent(Main.getPrefix() +
                                        Main.getMSG("Messages.Report-System.Notify.Team.Claimed").replace("%target%", target.getName())));

                                    TextComponent tc =
                                        new TextComponent(Main.getPrefix() + "§7Server §8» §c" + target.getServer().getInfo().getName());
                                    tc.setHoverEvent(new HoverEvent(Action.SHOW_TEXT,
                                        new Text("§7Username §8» §c" + target.getName() + "\n§7Ping §8» §c" + target.getPing() + "ms "
                                            + "\n\n§7Version §8» §c" + VersionDetector.getClientProtocol(target.getPendingConnection()))));
                                    player.sendMessage(tc);

                                    player.sendMessage(new TextComponent(
                                        Main.getPrefix() + "§cFinish the report with: §7/report confirm §8| §7/report cancel"));
                                    player.sendMessage(new TextComponent(""));
                                ...
                                player.sendMessage(
                                    new TextComponent(Main.getPrefix() + Main.getMSG("Messages.Report-System.Target-offline")));
                            }
                        } else {
                            player.sendMessage(
                                new TextComponent(Main.getPrefix() + Main.getMSG("Messages.Report-System.Currently-Working")));
                        }
                ...
                if (player.hasPermission(Main.getPermissionNotice("Permissions.Everything")) ||
                    ...
                    if (ReportManager.claimed.containsKey(player.getUniqueId().toString())) {
                        if (args[0].equalsIgnoreCase("confirm")) {
                            ...
                            player.sendMessage(
                                new TextComponent(Main.getPrefix() + Main.getMSG("Messages.Report-System.Report-Confirmed")));

                        } else if (args[0].equalsIgnoreCase("cancel")) {

                            ReportManager.cancelReport(player.getUniqueId().toString());
                            player.sendMessage(
                                new TextComponent(Main.getPrefix() + Main.getMSG("Messages.Report-System.Report-Cancelled")));
                            ...
                            if (args[0].equalsIgnoreCase("templates")) {
                                ...
                                if (!reasons.isEmpty()) {
                                    ...
                                    for (Reason reason : reasons) {
                                        player.sendMessage(new TextComponent(
                                            "§8- §c" + reason.getType().toString() + " §8| §7#" + reason.getId() + " §e" +
                                                reason.getName()));
                                    }
                                    player.sendMessage(new TextComponent(""));
                                ...
                                player.sendMessage(
                                    new TextComponent(Main.getPrefix() + Main.getMSG("Messages.Report-System.Usage.Command")));
                                player.sendMessage(
                                    new TextComponent(Main.getPrefix() + Main.getMSG("Messages.Report-System.Usage.Template")));
