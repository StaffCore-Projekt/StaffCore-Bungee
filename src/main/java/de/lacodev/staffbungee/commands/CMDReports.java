        ...
        if (sender instanceof ProxiedPlayer) {
            ...
            if (Main.getMySQL().isConnected()) {
                ...
                if (player.hasPermission(Main.getPermissionNotice("Permissions.Everything")) ||
                    player.hasPermission(Main.getPermissionNotice("Permissions.Reports.See"))) {
                    ...
                    if (args.length == 0) {
                        ...
                        try {
                            ...
                            if (!openReports.isEmpty()) {
                                ...
                                player.sendMessage(
                                    new TextComponent(Main.getPrefix() + Main.getMSG("Messages.Report-System.Reports.Title")));
                                ...
                                for (String uuid : openReports) {
                                    ...
                                    tc.setHoverEvent(
                                        new HoverEvent(net.md_5.bungee.api.chat.HoverEvent.Action.SHOW_TEXT, new Text(hovertext)));
                                ...
                                player.sendMessage(
                                    new TextComponent(Main.getPrefix() + Main.getMSG("Messages.Report-System.Reports.No-Open")));
                            }
                    ...
                    player.sendMessage(new TextComponent(Main.getPrefix() + Main.getMSG("Messages.System.No-Permission")
                        .replace("%permission%", Main.getPermissionNotice("Permissions.Reports.See"))));
                }
            ...
            try {
                sender.sendMessage(new TextComponent(
                    Main.getPrefix() + "§7Currently there are §c" + ReportManager.getOpenReports().size() + " Reports §7open!"));
            } catch (SQLException e) {
                e.printStackTrace();
            }
