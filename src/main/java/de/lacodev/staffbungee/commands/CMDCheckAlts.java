        ...
        if (sender instanceof ProxiedPlayer) {
            ...
            if (Main.getMySQL().isConnected()) {
                if (player.hasPermission(Main.getPermissionNotice("Permissions.Everything")) ||
                    player.hasPermission(Main.getPermissionNotice("Permissions.CheckAlts.Use"))) {
                            ...
                            Main.getInstance().getProxy().getScheduler().runAsync(Main.getInstance(), new Runnable() {
                                ...
                                public void run() {
                                    player.sendMessage(new TextComponent(Main.getPrefix() +
                                        Main.getMSG("Messages.Altaccount-Check.Collecting-Data")
                                            .replace("%ip%", PlayerManager.getLastKnownIp(player, uuid))));
                                    ...
                                    if (!alts.isEmpty()) {
                                        ...
                                        player.sendMessage(new TextComponent(Main.getPrefix() +
                                            Main.getMSG("Messages.Altaccount-Check.Possible-Alts-Title").replace("%target%", args[0])));
                                        ...
                                        for (String name : alts) {
                                            ...
                                            String listString = Main.getMSG("Messages.Player-Check.Prefix.Last-IP") +
                                                PlayerManager.getLastKnownIp(player, targetuuid) + "\n"
                                                + Main.getMSG("Messages.Player-Check.Prefix.Country") +
                                                PlayerManager.getCountry(targetuuid) + "\n"
                                                ...
                                                + Main.getMSG("Messages.Player-Check.Prefix.OnlineTime") +
                                                PlayerManager.getOnlineTime(targetuuid) + "\n \n"
                                                + Main.getMSG("Messages.Player-Check.Prefix.Reports") +
                                                PlayerManager.getReports(targetuuid) + "\n"
                                        ...
                                        player.sendMessage(
                                            new TextComponent(Main.getPrefix() + Main.getMSG("Messages.Altaccount-Check.Nothing-found")));
                                    }
                                }
                    ...
                    player.sendMessage(new TextComponent(Main.getPrefix() + Main.getMSG("Messages.System.No-Permission")
                        .replace("%permission%", Main.getPermissionNotice("Permissions.CheckAlts.Use"))));
                }
            } else {
                    ...
                    if (PlayerManager.existsPlayerData(uuid)) {
                        ...
                        Main.getInstance().getProxy().getScheduler().runAsync(Main.getInstance(), new Runnable() {
                            ...
                            public void run() {
                                player.sendMessage(new TextComponent(Main.getPrefix() +
                                    Main.getMSG("Messages.Altaccount-Check.Collecting-Data")
                                        .replace("%ip%", PlayerManager.getLastKnownIp(uuid))));
                                ...
                                if (!alts.isEmpty()) {
                                    ...
                                    for (String name : alts) {
                                        ...
                                        player.sendMessage(new TextComponent(""));
                                        player.sendMessage(new TextComponent(
                                            Main.getPrefix() + Main.getMSG("Messages.Altaccount-Check.Prefix.Username") + name));
                                    }
