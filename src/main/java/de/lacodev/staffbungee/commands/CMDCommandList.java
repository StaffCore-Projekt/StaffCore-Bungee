    ...
    public void execute(CommandSender sender, String[] args) {
        if (sender instanceof ProxiedPlayer) {
            ...
            if (player.hasPermission(Main.getPermissionNotice("Permissions.Everything")) ||
                player.hasPermission(Main.getPermissionNotice("Permissions.CommandList.Use"))) {
                    ...
                    if (args[0].equalsIgnoreCase("add")) {
                        ...
                        if (!CommandListManager.isListed(command)) {
                            ...
                            player.sendMessage(new TextComponent(Main.getPrefix() +
                                Main.getMSG("Messages.Mute.CommandList.Add-Success").replace("%command%", "/" + command)));
                        } else {
                            player.sendMessage(
                                new TextComponent(Main.getPrefix() + Main.getMSG("Messages.Mute.CommandList.Already-listed")));
                        }
                        ...
                        if (CommandListManager.isListed(command)) {
                            ...
                            player.sendMessage(new TextComponent(Main.getPrefix() +
                                Main.getMSG("Messages.Mute.CommandList.Remove-Success").replace("%command%", "/" + command)));
                        } else {
                ...
                player.sendMessage(new TextComponent(Main.getPrefix() + Main.getMSG("Messages.System.No-Permission")
                    .replace("%permission%", Main.getPermissionNotice("Permissions.CommandList.Use"))));
            }
                ...
                if (args[0].equalsIgnoreCase("add")) {
                    ...
                    if (!CommandListManager.isListed(command)) {
                        ...
                        player.sendMessage(new TextComponent(
                            Main.getPrefix() + Main.getMSG("Messages.Mute.CommandList.Add-Success").replace("%command%", "/" + command)));
                    } else {
