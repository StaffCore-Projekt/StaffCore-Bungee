        ...
        if (sender instanceof ProxiedPlayer) {
            ...
            if (Main.getMySQL().isConnected()) {
                if (player.hasPermission(Main.getPermissionNotice("Permissions.Everything")) ||
                    player.hasPermission(Main.getPermissionNotice("Permissions.PunishmentHistory.See"))) {
                    ...
                    if (args.length == 1) {
                        ...
                        if (PlayerManager.existsPlayerData(uuid)) {
                            ...
                            Main.getMySQL().update(
                                "INSERT INTO StaffCore_activitydb(type,uuid,target,message,reg_date,priority) VALUES ('PUNISHMENT_HISTORY_VIEWED','" +
                                    player.getUniqueId().toString() + "',"
                                    + "'" + uuid + "','%player% viewed the Punishment-History of %target%','" + System.currentTimeMillis() +
                                    "','1')");
                    ...
                    player.sendMessage(new TextComponent(Main.getPrefix() + Main.getMSG("Messages.System.No-Permission")
                        .replace("%permission%", Main.getPermissionNotice("Permissions.PunishmentHistory.See"))));
                }
            } else {
