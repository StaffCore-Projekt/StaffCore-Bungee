    ...
    public void execute(CommandSender sender, String[] args) {
        ...
        if (sender instanceof ProxiedPlayer) {
            ...
            if (args.length == 0) {
                ...
                player.sendMessage(
                    new TextComponent("§cSystem §8» StaffCore-Bungee: §7v" + Main.getInstance().getDescription().getVersion()));
                ...
                if (player.hasPermission(Main.getPermissionNotice("Permissions.Everything"))) {
                    ...
                    bugreport.setHoverEvent(
                        new HoverEvent(HoverEvent.Action.SHOW_TEXT, new Text("§7Click here to get help for StaffCore-Bungee")));
                    ...
                    if (SettingsManager.getValue(Settings.ADVERTISMENT_ENABLE).matches("FALSE")) {
                        ...
                        removeads.setHoverEvent(
                            new HoverEvent(HoverEvent.Action.SHOW_TEXT, new Text("§7Click here to toggle Advertisments for StaffCore")));
                    ...
                    if (player.hasPermission(Main.getPermissionNotice("Permissions.Everything"))) {
                        ...
                        try {
                            ...
                            if (setting != null) {
                                ...
                                player.sendMessage(new TextComponent(""));
                                player.sendMessage(
                                    new TextComponent(Main.getPrefix() + "§7You are now editing the Setting: §c" + setting.toString()));
                                player.sendMessage(new TextComponent(Main.getPrefix() + "§7Default: §c" + setting.getStandard()));
                                player.sendMessage(
                                    new TextComponent(Main.getPrefix() + "§7Your Chat is disabled! Until you type in §ccancel"));
                                player.sendMessage(new TextComponent(""));
