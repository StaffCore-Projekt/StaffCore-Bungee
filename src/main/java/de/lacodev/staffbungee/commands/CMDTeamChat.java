    ...
    public void execute(CommandSender sender, String[] args) {
        ...
        if (sender instanceof ProxiedPlayer) {
            ...
            if (player.hasPermission(Main.getPermissionNotice("Permissions.Everything")) ||
                player.hasPermission(Main.getPermissionNotice("Permissions.TeamChat.Use"))) {
                ...
                if (args.length == 1) {
                    ...
                    if (args[0].equalsIgnoreCase("login")) {
                        ...
                        Main.getMySQL().update(
                            "INSERT INTO StaffCore_activitydb(type,uuid,target,message,reg_date,priority) VALUES ('TEAMCHAT_JOINED','" +
                        ...
                        Main.getMySQL().update(
                            "INSERT INTO StaffCore_activitydb(type,uuid,target,message,reg_date,priority) VALUES ('TEAMCHAT_LEFT','" +
                                player.getUniqueId().toString() + "',"
                        ...
                        if (player.hasPermission(Main.getPermissionNotice("Permissions.Everything")) ||
                            player.hasPermission(Main.getPermissionNotice("Permissions.TeamChat.Ghost"))) {
                    ...
                    if (player.hasPermission(Main.getPermissionNotice("Permissions.Everything")) ||
                        player.hasPermission(Main.getPermissionNotice("Permissions.TeamChat.List"))) {
                        ...
                        for (ProxiedPlayer all : Main.getTeamChat().get()) {
                            if (Main.getTeamChat().isGhost(all)) {
                                if (player.hasPermission(Main.getPermissionNotice("Permissions.Everything")) ||
                                    ...
                                    tc2.setHoverEvent(new HoverEvent(Action.SHOW_TEXT,
                                        new Text("§7Server §8» §c" + all.getServer().getInfo().getName())));
                                    tc.addExtra(tc2);
                                }
                            } else {
                                TextComponent tc2 = new TextComponent();
                                ...
                                tc2.setHoverEvent(
                                    new HoverEvent(Action.SHOW_TEXT, new Text("§7Server §8» §c" + all.getServer().getInfo().getName())));
                                tc.addExtra(tc2);
                            }
                        }
                ...
                player.sendMessage(new TextComponent(Main.getPrefix() + Main.getMSG("Messages.System.No-Permission")
                    .replace("%permission%", Main.getPermissionNotice("Permissions.TeamChat.Use"))));
            }
            ...
            player.sendMessage(new TextComponent(
                Main.getPrefix() + "§7Currently there are §c" + Main.getTeamChat().get().size() + " User(s) §7in the teamchat!"));
        }
