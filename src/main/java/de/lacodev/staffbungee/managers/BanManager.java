...
public class BanManager {
    ...
    private static void addBan(String uuid) {
        if (Main.getMySQL().isConnected()) {
            Main.getMySQL()
                .update("UPDATE StaffCore_playerdb SET BANS = '" + (PlayerManager.getBans(uuid) + 1) + "' WHERE UUID = '" + uuid + "'");
        }
    }
    ...
    public static void ban(String targetuuid, String reason, String teamuuid) {
        ...
        if (!PlayerManager.isProtected(targetuuid)) {
            ...
            if (teamuuid != "Console") {
                ...
                ProxiedPlayer player = ProxyServer.getInstance().getPlayer(UUID.fromString(teamuuid));
                        ...
                        if (Main.getMySQL().isConnected()) {
                            if (ReasonManager.existsReason(ReasonType.BAN, reason)) {
                                ...
                                if (Main.getInstance().getConfig().getBoolean("ViolationLevelSystem.Enable")) {
                                    banEnd = ViolationLevelHandler.calculateLength(targetuuid, banReason.getLength(),
                                        ViolationLevelHandler.getVL(targetuuid));
                                ...
                                Main.getMySQL().update(
                                    "INSERT INTO StaffCore_bansdb(BAN_ID,BANNED_UUID,TEAM_UUID,REASON,BAN_END) VALUES ('" + banid + "','" +
                                        targetuuid + "','" + teamuuid + "','" + reason + "','" + banEnd + "')");
                                ...
                                ProxiedPlayer target = ProxyServer.getInstance().getPlayer(UUID.fromString(targetuuid));

                                if (target != null) {

                                    Main.getMySQL().update(
                                        "INSERT INTO StaffCore_punishmentsdb(TYPE,PUNISH_ID,UUID,TEAM_UUID,REASON,BAN_START,BAN_END,SUB_SERVER) VALUES ('BAN','" +
                                            banid + "','" + targetuuid + "','" + teamuuid + "','" + reason + "'"
                                    ...
                                    Main.getMySQL().update(
                                        "INSERT INTO StaffCore_punishmentsdb(TYPE,PUNISH_ID,UUID,TEAM_UUID,REASON,BAN_START,BAN_END) VALUES ('BAN','" +
                                            banid + "','" + targetuuid + "','" + teamuuid + "','" + reason + "'"
                                            + ",'" + System.currentTimeMillis() + "','" + banEnd + "')");
                                }

                                Main.getMySQL().update(
                                    "INSERT INTO StaffCore_activitydb(type,uuid,target,message,reg_date,priority) VALUES ('PLAYER_BANNED','" +
                                        teamuuid + "',"
                                        + "'" + PlayerManager.getUsernamebyUUID(targetuuid) +
                                        "','%player% banned %target% from the network','" + System.currentTimeMillis() + "','3')");

                                NotificationManager.sendNotify(NotificationSender.PLAYER_NOTIFY, "BAN",
                                    PlayerManager.getUsernamebyUUID(teamuuid), PlayerManager.getUsernamebyUUID(targetuuid), reason);
                    ...
                    public void run() {
                        if (Main.getMySQL().isConnected()) {
                            if (ReasonManager.existsReason(ReasonType.BAN, reason)) {
                                ...
                                NotificationManager.sendNotify(NotificationSender.CONSOLE_NOTIFY, "BAN",
                                    PlayerManager.getUsernamebyUUID(targetuuid), reason);

                            } else {
                                ProxyServer.getInstance().getConsole().sendMessage(new TextComponent(""));
                                ProxyServer.getInstance().getConsole()
                                    .sendMessage(new TextComponent("§cSystem §8» §c§lFAILED §8(§7Reason does not exist§8)"));
                                ProxyServer.getInstance().getConsole().sendMessage(new TextComponent(""));
                            }
                        } else {
                            ProxyServer.getInstance().getConsole().sendMessage(new TextComponent(""));
                            ProxyServer.getInstance().getConsole()
                                .sendMessage(new TextComponent("§cSystem §8» §c§lFAILED §8(§7MySQL Connection§8)"));
                            ProxyServer.getInstance().getConsole().sendMessage(new TextComponent(""));
                        }
                    }
    ...
    public static void unban(String targetuuid, String teamuuid, String reason) {
        ...
        if (teamuuid != "Console") {
            ...
            ProxiedPlayer team = ProxyServer.getInstance().getPlayer(UUID.fromString(targetuuid));
            ...
            Main.getInstance().getProxy().getScheduler().runAsync(Main.getInstance(), new Runnable() {
                ...
                public void run() {
                    if (Main.getMySQL().isConnected()) {
                        ...
                        Main.getMySQL()
                            .update("UPDATE StaffCore_punishmentsdb SET UNBANNED = '1', UNBAN_TIME = '" + System.currentTimeMillis() + "', "
                        ...
                        NotificationManager.sendNotify(NotificationSender.PLAYER_NOTIFY, "UNBAN", PlayerManager.getUsernamebyUUID(teamuuid),
                            PlayerManager.getUsernamebyUUID(targetuuid), reason);
            ...
            Main.getInstance().getProxy().getScheduler().runAsync(Main.getInstance(), new Runnable() {
                ...
                public void run() {
                    if (Main.getMySQL().isConnected()) {
                        ...
                        NotificationManager.sendNotify(NotificationSender.CONSOLE_NOTIFY, "UNBAN",
                            PlayerManager.getUsernamebyUUID(targetuuid), reason);
    ...
    public static void banIp(String targetuuid, String teamuuid) {
        ...
        if (!PlayerManager.isProtected(targetuuid)) {
            ...
            Main.getMySQL().update(
                "INSERT INTO StaffCore_ipbansdb(BANNED_UUID,IP_ADDRESS,TEAM_UUID,BAN_END) VALUES ('" + targetuuid + "','" + banned_ip +
                    "','" + teamuuid + "','" + banEnd + "')");
            ...
            if (target != null) {
                ...
                Main.getMySQL().update(
                    "INSERT INTO StaffCore_punishmentsdb(TYPE,UUID,TEAM_UUID,BAN_START,BAN_END,SUB_SERVER) VALUES ('IPBAN','" + targetuuid +
                        "','" + teamuuid + "'"
                        + ",'" + System.currentTimeMillis() + "','" + banEnd + "','" + target.getServer().getInfo().getName() + "')");

            } else {
                Main.getMySQL().update(
                    "INSERT INTO StaffCore_punishmentsdb(TYPE,UUID,TEAM_UUID,BAN_START,BAN_END) VALUES ('IPBAN','" + targetuuid + "','" +
                        teamuuid + "'"
                        + ",'" + System.currentTimeMillis() + "','" + banEnd + "')");
            }
            ...
            NotificationManager.sendIpBanNotify(targetuuid, teamuuid,
                (Main.getInstance().getConfig().getLong("IPBan.Duration-In-Hours") * 1000 * 60 * 60));
