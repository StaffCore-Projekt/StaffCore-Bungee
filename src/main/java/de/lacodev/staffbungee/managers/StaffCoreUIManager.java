...
public class StaffCoreUIManager {
    ...
    public static void verify(ProxiedPlayer player) {
        Main.getMySQL().update("UPDATE StaffCoreUI_Accounts SET uuid = '" + player.getUniqueId().toString() + "' WHERE username = '" +
        ...
        ProxyServer.getInstance().getScheduler().runAsync(Main.getInstance(), new Runnable() {
            ...
            public void run() {
                ResultSet rs = Main.getMySQL().query(
                    "SELECT id,TYPE,PROCESS FROM StaffCoreUI_Sync_OnJoin WHERE USERNAME = '" + player.getDisplayName() + "' ORDER BY id");
