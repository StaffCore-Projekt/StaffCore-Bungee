...
public class PlayerManager {
                ...
                if (Main.getMySQL().isConnected()) {
                    if (!existsPlayerData(player.getUniqueId().toString())) {
                        ...
                        Main.getMySQL().update(
                            "INSERT INTO StaffCore_playerdb(UUID,PLAYERNAME,BANS,MUTES,REPORTS,WARNS,LAST_KNOWN_IP,FIRST_LOGIN,LAST_LOGIN,PROTECTED,COUNTRY,REGION,ONLINE) "
                                + "VALUES ('" + player.getUniqueId().toString() + "','" + player.getDisplayName() + "','0','0','0','0',"
                                + "'" + ip.substring(1, ip.length() - 6) + "','" + System.currentTimeMillis() + "','" +
                                System.currentTimeMillis() + "','0',"
                                + "'" + IPLookup.getCountry(ip.substring(1, ip.length() - 6)) + "','" +
                                IPLookup.getRegion(ip.substring(1, ip.length() - 6)) + "','1')");

                        Main.getMySQL().update(
                            "INSERT INTO StaffCore_violationleveldb(UUID,VL) VALUES ('" + player.getUniqueId().toString() + "','0')");
                        Main.getMySQL().update(
                            "INSERT INTO StaffCore_violationresetdb(UUID,NEXT_RESET) VALUES ('" + player.getUniqueId().toString() +
                                "','0')");
    ...
    private static void updatePlayerData(ProxiedPlayer player) {
        ...
        if (!ViolationLevelHandler.hasVL(player.getUniqueId().toString())) {
            Main.getMySQL()
                .update("INSERT INTO StaffCore_violationleveldb(UUID,VL) VALUES ('" + player.getUniqueId().toString() + "','0')");
        }
        if (!ViolationLevelHandler.hasLastReset(player.getUniqueId().toString())) {
            Main.getMySQL()
                .update("INSERT INTO StaffCore_violationresetdb(UUID,NEXT_RESET) VALUES ('" + player.getUniqueId().toString() + "','0')");
        }
    ...
    public static String getOnlineTime(String uuid) {
        if (Main.getMySQL().isConnected()) {
            ResultSet rs = Main.getMySQL().query(
                "SELECT SUM(SESSION_END - SESSION_START) as playtime FROM StaffCore_sessionsdb WHERE UUID = '" + uuid +
                    "' AND FINISHED = '1'");
    ...
    public static ArrayList<Session> getSessions(String uuid) throws SQLException {
        ...
        if (Main.getMySQL().isConnected()) {
            ...
            ResultSet rs = Main.getMySQL()
                .query("SELECT * FROM StaffCore_sessionsdb WHERE UUID = '" + uuid + "' AND FINISHED = '1' ORDER BY id DESC LIMIT 10");
    ...
    public static ArrayList<Punishment> getPunishments(String uuid) throws SQLException {
        ...
        if (Main.getMySQL().isConnected()) {
            ...
            ResultSet rs =
                Main.getMySQL().query("SELECT * FROM StaffCore_punishmentsdb WHERE UUID = '" + uuid + "' ORDER BY id DESC LIMIT 10");

            while (rs.next()) {
                punishments.add(new Punishment(rs.getInt("id"), PunishmentType.valueOf(rs.getString("TYPE")), rs.getString("REASON"),
                    rs.getString("UUID"),
                    rs.getString("TEAM_UUID"), rs.getLong("BAN_START"), rs.getLong("BAN_END"), rs.getString("SUB_SERVER"),
                    rs.getBoolean("UNBANNED"),
    ...
    public static ArrayList<Punishment> getPunishments(String uuid, PunishmentType type) throws SQLException {
        ...
        if (Main.getMySQL().isConnected()) {
            ...
            ResultSet rs = Main.getMySQL().query(
                "SELECT * FROM StaffCore_punishmentsdb WHERE UUID = '" + uuid + "' AND TYPE = '" + type.toString() +
    ...
    public static Punishment getPunishment(int id) throws SQLException {
        ...
        if (Main.getMySQL().isConnected()) {
            ...
            while (rs.next()) {
                punishments = new Punishment(rs.getInt("id"), PunishmentType.valueOf(rs.getString("TYPE")), rs.getString("REASON"),
                    rs.getString("UUID"),
                    rs.getString("TEAM_UUID"), rs.getLong("BAN_START"), rs.getLong("BAN_END"), rs.getString("SUB_SERVER"),
                    rs.getBoolean("UNBANNED"),
                    rs.getLong("UNBAN_TIME"), rs.getString("UNBAN_REASON"), rs.getString("UNBAN_STAFF"));
