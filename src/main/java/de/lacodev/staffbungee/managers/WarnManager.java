...
public class WarnManager {
                ...
                public void run() {
                    ...
                    if (teamuuid != "Console") {
                        ...
                        if (Main.getMySQL().isConnected()) {
                            ...
                            if (target != null) {
                                Main.getMySQL()
                                    .update("INSERT INTO StaffCore_warnsdb(WARNED_UUID,TEAM_UUID,REASON,WARNED_AT,SUB_SERVER) VALUES "
                                ...
                                Main.getMySQL()
                                    .update("INSERT INTO StaffCore_punishmentsdb(TYPE,UUID,TEAM_UUID,REASON,BAN_START,SUB_SERVER) VALUES "
                                        + "('WARN','" + targetuuid + "','" + teamuuid + "','" + reason + "','" +
                                        System.currentTimeMillis() + "','" + target.getServer().getInfo().getName() + "')");
                            ...
                            NotificationManager.sendNotify(NotificationSender.PLAYER_NOTIFY, "WARN", player.getName(),
                        ...
                        if (Main.getMySQL().isConnected()) {
                            ...
                            NotificationManager.sendNotify(NotificationSender.CONSOLE_NOTIFY, "WARN",
                                PlayerManager.getUsernamebyUUID(targetuuid), reason);
    ...
    public static void addWarn(String uuid) {
        if (Main.getMySQL().isConnected()) {
            Main.getMySQL()
                .update("UPDATE StaffCore_playerdb SET WARNS = '" + (PlayerManager.getWarns(uuid) + 1) + "' WHERE UUID = '" + uuid + "'");
        }
    }
