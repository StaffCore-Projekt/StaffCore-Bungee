...
public class ReportManager {
        ...
        Main.getInstance().getProxy().getScheduler().runAsync(Main.getInstance(), new Runnable() {
            ...
            public void run() {
                ...
                if (Main.getMySQL().isConnected()) {
                    ...
                    Main.getMySQL()
                        .update("INSERT INTO StaffCore_reportsdb(TYPE,UUID,REPORTER_UUID,REASON,STATUS,CREATED_AT,UPDATED_AT) VALUES "
                            + "('" + type.toString() + "','" + targetuuid + "','" + reporteruuid + "','" + reason + "','" +
                            ReportStatus.CREATED.getStatus() + "'"
                            + ",'" + System.currentTimeMillis() + "','" + System.currentTimeMillis() + "')");
    ...
    public static boolean hasOpenReports(String targetuuid) {
        ...
        if (Main.getMySQL().isConnected()) {
            ...
            ResultSet rs = Main.getMySQL().query("SELECT STATUS FROM StaffCore_reportsdb WHERE UUID = '" + targetuuid + "' AND STATUS = '" +
                ReportStatus.CREATED.getStatus() + "'");
            ...
            public void run() {
                ...
                if (Main.getMySQL().isConnected()) {
                    ...
                    Main.getMySQL().update(
                        "UPDATE StaffCore_reportsdb SET TEAM_UUID = '" + teamuuid + "',STATUS = '" + ReportStatus.CLAIMED.getStatus() + "',"
                    ...
                    try {
                        ...
                        if (!reporters.isEmpty()) {
                            ...
                            for (String uuid : reporters) {
                                ...
                                ProxiedPlayer reporter = ProxyServer.getInstance().getPlayer(UUID.fromString(uuid));

                                if (reporter != null) {
                                    reporter.sendMessage(new TextComponent(
                                        Main.getPrefix() + Main.getMSG("Messages.Report-System.Notify.User.Team-Claimed-Report")));
                                }
    ...
    private static ArrayList<String> getReporters(String targetuuid) throws SQLException {
        ...
        if (Main.getMySQL().isConnected()) {
            ...
            ResultSet rs = Main.getMySQL().query(
                "SELECT REPORTER_UUID FROM StaffCore_reportsdb WHERE UUID = '" + targetuuid + "' AND TYPE = '" +
                    ReportType.PLAYER_REPORT.toString() + "'");
    ...
    public static ArrayList<String> getOpenReports() throws SQLException {
        ...
        if (Main.getMySQL().isConnected()) {
            ...
            ResultSet rs =
                Main.getMySQL().query("SELECT UUID FROM StaffCore_reportsdb WHERE STATUS = '" + ReportStatus.CREATED.getStatus() + "'");
    ...
    private static void addReport(String uuid) {
        if (Main.getMySQL().isConnected()) {
            Main.getMySQL().update(
                "UPDATE StaffCore_playerdb SET REPORTS = '" + (PlayerManager.getReports(uuid) + 1) + "' WHERE UUID = '" + uuid + "'");
        }
    }
