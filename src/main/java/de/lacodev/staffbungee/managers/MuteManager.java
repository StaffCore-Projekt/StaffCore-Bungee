...
public class MuteManager {
    ...
    private static void addMute(String uuid) {
        if (Main.getMySQL().isConnected()) {
            Main.getMySQL()
                .update("UPDATE StaffCore_playerdb SET MUTES = '" + (PlayerManager.getMutes(uuid) + 1) + "' WHERE UUID = '" + uuid + "'");
        }
    }
                        ...
                        if (Main.getMySQL().isConnected()) {
                            if (ReasonManager.existsReason(ReasonType.MUTE, reason)) {
                                ...
                                if (Main.getInstance().getConfig().getBoolean("ViolationLevelSystem.Enable")) {
                                    muteEnd = ViolationLevelHandler.calculateLength(targetuuid, muteReason.getLength(),
                                ...
                                Main.getMySQL().update(
                                    "INSERT INTO StaffCore_mutesdb(MUTE_ID,MUTED_UUID,TEAM_UUID,REASON,MUTE_END) VALUES ('" + muteid +
                                        "','" + targetuuid + "','" + teamuuid + "','" + reason + "','" + muteEnd + "')");
                                ...
                                if (target != null) {
                                    ...
                                    Main.getMySQL().update(
                                        "INSERT INTO StaffCore_punishmentsdb(TYPE,PUNISH_ID,UUID,TEAM_UUID,REASON,BAN_START,BAN_END,SUB_SERVER) VALUES ('MUTE','" +
                                            muteid + "','" + targetuuid + "','" + teamuuid + "','" + reason + "'"
                                            + ",'" + System.currentTimeMillis() + "','" + muteEnd + "','" +
                                            target.getServer().getInfo().getName() + "')");

                                    target.sendMessage(new TextComponent(
                                        Main.getMSG("Messages.Mute-System.Message-If-Player-Muted").replace("%reason%", reason)));

                                } else {
                                    Main.getMySQL().update(
                                        "INSERT INTO StaffCore_punishmentsdb(TYPE,MUTE_ID,UUID,TEAM_UUID,REASON,BAN_START,BAN_END) VALUES ('MUTE','" +
                                            muteid + "','" + targetuuid + "','" + teamuuid + "','" + reason + "'"
                                            + ",'" + System.currentTimeMillis() + "','" + muteEnd + "')");
                                }

                                Main.getMySQL().update(
                                    "INSERT INTO StaffCore_activitydb(type,uuid,target,message,reg_date,priority) VALUES ('PLAYER_MUTED','" +
                                        teamuuid + "',"
                                        + "'" + PlayerManager.getUsernamebyUUID(targetuuid) + "','%player% muted %target%','" +
                                        System.currentTimeMillis() + "','3')");

                                NotificationManager.sendNotify(NotificationSender.PLAYER_NOTIFY, "MUTE",
                        ...
                        if (Main.getMySQL().isConnected()) {
                            if (ReasonManager.existsReason(ReasonType.MUTE, reason)) {
                                ...
                                if (target != null) {
                                    ...
                                    Main.getMySQL().update(
                                        "INSERT INTO StaffCore_punishmentsdb(TYPE,PUNISH_ID,UUID,TEAM_UUID,REASON,BAN_START,BAN_END) VALUES ('MUTE','" +
                                            muteid + "','" + targetuuid + "','" + teamuuid + "','" + reason + "'"
                                            + ",'" + System.currentTimeMillis() + "','" + muteEnd + "')");
                                }

                                NotificationManager.sendNotify(NotificationSender.CONSOLE_NOTIFY, "MUTE",
            ...
            Main.getInstance().getProxy().getScheduler().runAsync(Main.getInstance(), new Runnable() {
                ...
                public void run() {
                    if (Main.getMySQL().isConnected()) {
                        ...
                        NotificationManager.sendNotify(NotificationSender.PLAYER_NOTIFY, "UNMUTE",
                            PlayerManager.getUsernamebyUUID(teamuuid), PlayerManager.getUsernamebyUUID(targetuuid), reason);
            ...
            Main.getInstance().getProxy().getScheduler().runAsync(Main.getInstance(), new Runnable() {
                ...
                public void run() {
                    if (Main.getMySQL().isConnected()) {
                        ...
                        NotificationManager.sendNotify(NotificationSender.CONSOLE_NOTIFY, "UNMUTE",
                            PlayerManager.getUsernamebyUUID(targetuuid), reason);
