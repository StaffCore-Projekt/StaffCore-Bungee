                            ...
                            while (rs.next()) {
                                ...
                                switch (rs.getString("TYPE")) {
                                    case "VERIFY_ACCOUNT":
                                        ...
                                        if (player != null) {
                                            ...
                                            Main.getMySQL().update(
                                                "INSERT INTO StaffCoreUI_Sync_OnJoin(USERNAME,TYPE,PROCESS) VALUES ('" + username +
                                                    "','VERIFY_ACCOUNT','" + rs.getString("PROCESS") + "')");
                                            Main.getMySQL().update("DELETE FROM StaffCoreUI_Sync WHERE id = '" + rs.getInt("id") + "'");
                                        }
                                        break;
                                    case "WATCHLIST_ADD":
                                    ...
                                    case "BAN_PLAYER":
                                        ...
                                        if (!silent) {
                                            NotificationManager.sendNotify(NotificationSender.PLAYER_NOTIFY, type,
                                                PlayerManager.getUsernamebyUUID(teamuuid), PlayerManager.getUsernamebyUUID(targetuuid),
                                                reason);
                                        }
                                        Main.getMySQL().update("DELETE FROM StaffCoreUI_Sync WHERE id = '" + rs.getInt("id") + "'");
                                        break;
                                    case "MUTE_PLAYER":
                                        ...
                                        ProxiedPlayer target1 = ProxyServer.getInstance().getPlayer(UUID.fromString(targetuuid1));

                                        if (target1 != null) {

                                            target1.sendMessage(new TextComponent(
                                                Main.getMSG("Messages.Mute-System.Message-If-Player-Muted").replace("%reason%", reason1)));

                                        }

                                        if (!silent1) {
                                            NotificationManager.sendNotify(NotificationSender.PLAYER_NOTIFY, type1,
                                                PlayerManager.getUsernamebyUUID(teamuuid1), PlayerManager.getUsernamebyUUID(targetuuid1),
                                                reason1);
                                        }
                                        Main.getMySQL().update("DELETE FROM StaffCoreUI_Sync WHERE id = '" + rs.getInt("id") + "'");
                                        break;
                                    case "UNBAN_PLAYER":
                                        ...
                                        NotificationManager.sendNotify(NotificationSender.PLAYER_NOTIFY, "UNBAN",
                                            PlayerManager.getUsernamebyUUID(teamuuid11), PlayerManager.getUsernamebyUUID(targetuuid11),
                                            reason11);
                                    ...
                                    case "UNMUTE_PLAYER":
                                        ...
                                        NotificationManager.sendNotify(NotificationSender.PLAYER_NOTIFY, "UNMUTE",
                                            PlayerManager.getUsernamebyUUID(teamuuid111), PlayerManager.getUsernamebyUUID(targetuuid111),
                                            reason111);
                                    ...
                                    case "JOIN_SERVER":
                                        ...
                                        ProxiedPlayer target11 = ProxyServer.getInstance().getPlayer(UUID.fromString(targetuuid1111));

                                        if (target11 != null) {

                                            ProxiedPlayer player1 = ProxyServer.getInstance().getPlayer(UUID.fromString(teamuuid1111));

                                            player1.connect(target11.getServer().getInfo());

                                            player1.sendMessage(new TextComponent(""));
                                            player1.sendMessage(new TextComponent(Main.getPrefix() +
                                                Main.getMSG("Messages.Report-System.Notify.Team.Claimed")
                                                    .replace("%target%", target11.getName())));

                                            TextComponent tc = new TextComponent(
                                                Main.getPrefix() + "§7Server §8» §c" + target11.getServer().getInfo().getName());
                                            tc.setHoverEvent(new HoverEvent(Action.SHOW_TEXT, new Text(
                                                "§7Username §8» §c" + target11.getName() + "\n§7Ping §8» §c" + target11.getPing() + "ms "
                                                    + "\n\n§7Version §8» §c" +
                                                    VersionDetector.getClientProtocol(target11.getPendingConnection()))));
