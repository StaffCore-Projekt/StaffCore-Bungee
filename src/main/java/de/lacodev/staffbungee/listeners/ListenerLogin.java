            ...
            for (ProxiedPlayer all : Main.getInstance().getProxy().getPlayers()) {
                if (all.hasPermission(Main.getPermissionNotice("Permissions.Everything")) ||
                    ...
                    all.sendMessage(new TextComponent(
                        Main.getMSG("Messages.System.Alerts.MCLeaks-AccountBlock").replace("%target%", e.getConnection().getName())));
                }
            }
    ...
    public void onPostLogin(PostLoginEvent e) throws Exception {
        ...
        if (Main.getMaintenanceHandler().isMaintenance()) {
            ...
            if (!player.hasPermission(Main.getPermissionNotice("Permissions.Everything")) &&
                !player.hasPermission(Main.getPermissionNotice("Permissions.Maintenance.Bypass"))) {

                e.getPlayer().getPendingConnection().disconnect(new TextComponent(
            ...
            if (IPLookup.isUsingProxy(PlayerManager.getLastKnownIp(player.getUniqueId().toString()))) {
                for (ProxiedPlayer all : Main.getInstance().getProxy().getPlayers()) {
                    if (all.hasPermission(Main.getPermissionNotice("Permissions.Everything")) ||
                        all.hasPermission(Main.getPermissionNotice("Permissions.System.Notify"))) {
                        all.sendMessage(new TextComponent(
                            Main.getMSG("Messages.System.Alerts.VPN-Detected").replace("%target%", player.getDisplayName())));
                    }
                }
                Main.getInstance().getProxy().getConsole().sendMessage(
                    new TextComponent(Main.getMSG("Messages.System.Alerts.VPN-Detected").replace("%target%", player.getDisplayName())));

                if (Main.getInstance().getConfig().getBoolean("VPN-Detection.Allow-VPN-Join") == false) {
                    if (!(player.hasPermission(Main.getPermissionNotice("Permissions.Everything")) ||
                        player.hasPermission(Main.getPermissionNotice("Permissions.System.VPN-Block.Bypass")))) {
            ...
            if (player.isForgeUser()) {
                for (ProxiedPlayer all : Main.getInstance().getProxy().getPlayers()) {
                    if (all.hasPermission(Main.getPermissionNotice("Permissions.Everything")) ||
                        all.hasPermission(Main.getPermissionNotice("Permissions.System.Notify"))) {
                        all.sendMessage(new TextComponent(
                            Main.getMSG("Messages.System.Alerts.Forge-Detected").replace("%target%", player.getDisplayName())));
                    }
                }
                Main.getInstance().getProxy().getConsole().sendMessage(
                    new TextComponent(Main.getMSG("Messages.System.Alerts.Forge-Detected").replace("%target%", player.getDisplayName())));
            }
        }
        ...
        if (player.hasPermission(Main.getPermissionNotice("Permissions.Everything"))) {
            if (!Main.getInstance().latest) {
                if (Main.getInstance().getDescription().getVersion().endsWith("Pre")) {
                    ...
                    player.sendMessage(
                        new TextComponent("§cSystem §8» §7Download the latest version (" + Main.getInstance().version + ") here:"));
