    ...
    public void onChat(ChatEvent e) {
        ...
        if (CMDStaffCore.settings.containsKey(player)) {
            if (!e.getMessage().matches("cancel")) {
                ...
                if (CMDStaffCore.settings.get(player).equals(Settings.MOTD_ENABLE) ||
                    CMDStaffCore.settings.get(player).equals(Settings.MOTD_FAKEPLAYERS_ENABLE) ||
                    CMDStaffCore.settings.get(player).equals(Settings.ADVERTISMENT_ENABLE)) {
        ...
        if (!MuteManager.isMuted(uuid)) {
            ...
            Main.getMySQL().update("INSERT INTO StaffCore_messages(SENDER_UUID,MESSAGE) VALUES ('" + uuid + "','" +
                StringGenerator.getMySQLFriendly(e.getMessage()) + "')");
            ...
            if (e.getMessage().startsWith("/")) {
                    ...
                    for (ProxiedPlayer staff : ProxyServer.getInstance().getPlayers()) {
                        if (Main.getCommandSpy().isSpying(staff)) {
                            ...
                            if (Main.getCommandSpy().getSpyingServer(staff).matches("Global")) {
                                ...
                                staff.sendMessage(new TextComponent(Main.getPrefix() +
                                    Main.getMSG("Messages.CommandSpy.Command-Executed").replace("%target%", player.getName())
                                        .replace("%command%", e.getMessage())));
                                staff.sendMessage(
                                    new TextComponent(Main.getPrefix() + "§7Server §8» §c" + player.getServer().getInfo().getName()));
                ...
                if (!(player.hasPermission(Main.getPermissionNotice("Permissions.Everything")) ||
                    player.hasPermission(Main.getPermissionNotice("Permissions.ChatFilter.Bypass")))) {
            ...
            if (!e.getMessage().startsWith("/")) {
                ...
                if (MuteManager.getRawEnd(uuid) != -1) {
                    ...
                    player.sendMessage(new TextComponent(
                        Main.getMSG("Messages.Layouts.Mute").replace("%reason%", MuteManager.getReason(uuid))
                            .replace("%remaining%", MuteManager.getFormattedEnd(uuid))));
                    e.setCancelled(true);
