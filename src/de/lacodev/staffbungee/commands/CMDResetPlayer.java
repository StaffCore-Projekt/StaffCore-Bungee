...
public class CMDResetPlayer extends Command {
	...
	public void execute(CommandSender sender, String[] args) {
		...
		if(sender instanceof ProxiedPlayer) {
			...
			if(player.hasPermission(Main.getPermissionNotice("Permissions.Everything")) || player.hasPermission(Main.getPermissionNotice("Permissions.System.ResetPlayer"))) {
					...
					if(uuid != null) {
						...
						if(args[0].equalsIgnoreCase("reports")) {
							...
							if(BungeeCord.getInstance().getPlayer(args[1]) != null) {
								BungeeCord.getInstance().getPlayer(args[1]).disconnect(new TextComponent(Main.getPrefix() + Main.getMSG("Messages.PlayerReset.KickMessage")));
							}
							
						} else {
							player.sendMessage(new TextComponent(Main.getPrefix() + "§7/resetplayer <Reports / P-History / Sessions / §7Messages / ALL> <Player>"));
						}
				...
				player.sendMessage(new TextComponent(Main.getPrefix() + Main.getMSG("Messages.System.No-Permission").replace("%permission%", Main.getPermissionNotice("Permissions.System.ResetPlayer"))));
			}
