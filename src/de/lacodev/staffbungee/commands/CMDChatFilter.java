...
public class CMDChatFilter extends Command {
	...
	public void execute(CommandSender sender, String[] args) {
		...
		if(sender instanceof ProxiedPlayer) {
			...
			if(player.hasPermission(Main.getPermissionNotice("Permissions.Everything")) || player.hasPermission(Main.getPermissionNotice("Permissions.ChatFilter.Change"))) {
					...
					if(args[0].equalsIgnoreCase("add")) {
						...
						if(!ChatDetector.existsSwearingWord(args[1])) {
							...
							Main.getInstance().getProxy().getScheduler().runAsync(Main.getInstance(), new Runnable() {
								...
								@Override
								public void run() {
									Main.getMySQL().update("INSERT INTO StaffCore_swearingdb(WORD,ADDED_AT) VALUES ('"+ args[1] +"','"+ System.currentTimeMillis() +"')");
				...
				player.sendMessage(new TextComponent(Main.getPrefix() + Main.getMSG("Messages.System.No-Permission").replace("%permission%", Main.getPermissionNotice("Permissions.ChatFilter.Change"))));
			}
		}
