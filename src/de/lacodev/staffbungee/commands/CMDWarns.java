	...
	public void execute(CommandSender sender, String[] args) {
		...
		if(sender instanceof ProxiedPlayer) {
			...
			if(Main.getMySQL().isConnected()) {
				...
				if(player.hasPermission(Main.getPermissionNotice("Permissions.Everything")) || player.hasPermission(Main.getPermissionNotice("Permissions.Warns.See"))) {
						...
						if(uuid != null) {
							...
							Main.getInstance().getProxy().getScheduler().runAsync(Main.getInstance(), new Runnable() {
								...
								public void run() {
									...
									player.sendMessage(new TextComponent(Main.getPrefix() + Main.getMSG("Messages.Warn-System.Warns.Player-Info").replace("%target%", args[0]).replace("%warns%", "" + PlayerManager.getWarns(uuid))));
									player.sendMessage(new TextComponent(""));
									try {
										...
										if(!warns.isEmpty()) {
											for(Punishment warn : warns) {
												...
												tc.setText("§8- §c" + warn.getType().toString() + " §8| §e" + warn.getReason() + " §8| §7" + loginLastDate);
												tc.setHoverEvent(new HoverEvent(Action.SHOW_TEXT, new Text("§7#" + warn.getId() + "\n\n§7Reason §8» §c" + warn.getReason() + "\n§7Server §8» §c"+ warn.getServer()
												+ "\n\n§7Warned by §8» §c" + warn.getPunisher() + "\n\n§7Warned at §8» §c" + loginLastDate)));
					...
					player.sendMessage(new TextComponent(Main.getPrefix() + Main.getMSG("Messages.System.No-Permission").replace("%permission%", Main.getPermissionNotice("Permissions.Warns.See"))));
				}
