...
public class CMDBanIp extends Command {
	...
	public void execute(CommandSender sender, String[] args) {
		...
		if(sender instanceof ProxiedPlayer) {
			...
			if(player.hasPermission(Main.getPermissionNotice("Permissions.Everything")) || player.hasPermission(Main.getPermissionNotice("Permissions.IpBan.Use"))) {
				...
				if(args.length == 1) {
					...
					if(uuid != null) {
						...
						if(!BanManager.isIpBanned(PlayerManager.getLastKnownIp(uuid))) {
							...
							player.sendMessage(new TextComponent(Main.getPrefix() + Main.getMSG("Messages.Ban-System.IP-Ban.Already-banned").replace("%target%", args[0])));
						}
				...
				player.sendMessage(new TextComponent(Main.getPrefix() + Main.getMSG("Messages.System.No-Permission").replace("%permission%", Main.getPermissionNotice("Permissions.IpBan.Use"))));
			}
