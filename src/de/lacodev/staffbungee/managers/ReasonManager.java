...
public class ReasonManager {
	...
	public static void addReason(ReasonType type, String name, String timeunit, int length) {
		...
		case BAN:
			...
			Main.getMySQL().update("INSERT INTO StaffCore_reasonsdb(TYPE,NAME,BAN_LENGTH) VALUES ('BAN','"+ name +"','"+ (length * btime) +"')");
		...
		case MUTE:
			...
			Main.getMySQL().update("INSERT INTO StaffCore_reasonsdb(TYPE,NAME,BAN_LENGTH) VALUES ('MUTE','"+ name +"','"+ (mtime * length) +"')");
	...
	public static Reason getReasonByName(ReasonType type, String name) {
		if(Main.getMySQL().isConnected()) {
			if(type.equals(ReasonType.REPORT)) {
				ResultSet rs = Main.getMySQL().query("SELECT * FROM StaffCore_reportreasonsdb WHERE NAME = '"+ name +"' AND TYPE = '"+ type.toString() +"'");
				...
				ResultSet rs = Main.getMySQL().query("SELECT * FROM StaffCore_reasonsdb WHERE NAME = '"+ name +"' AND TYPE = '"+ type.toString() +"'");
	...
	public static boolean existsReason(ReasonType type, String name) {
		if(Main.getMySQL().isConnected()) {
			if(type.equals(ReasonType.REPORT)) {
				ResultSet rs = Main.getMySQL().query("SELECT * FROM StaffCore_reportreasonsdb WHERE TYPE = '"+ type.toString() +"' AND NAME = '"+ name +"'");
				...
				ResultSet rs = Main.getMySQL().query("SELECT * FROM StaffCore_reasonsdb WHERE TYPE = '"+ type.toString() +"' AND NAME = '"+ name +"'");
	...
	public static ArrayList<Reason> getReasons() {
		...
		if(Main.getMySQL().isConnected()) {
			...
			try {
				while(rs.next()) {
					reasons.add(new Reason(ReasonType.valueOf(rs.getString("TYPE")), rs.getInt("id"), rs.getString("NAME"), rs.getLong("BAN_LENGTH"), rs.getBoolean("ADMIN_BAN")));
				}
				return reasons;
			} catch (SQLException e) {
				e.printStackTrace();
	...
	public static ArrayList<Reason> getReasons(ReasonType type) {
		if(type.equals(ReasonType.REPORT)) {
			...
			if(Main.getMySQL().isConnected()) {
				ResultSet rs = Main.getMySQL().query("SELECT * FROM StaffCore_reportreasonsdb WHERE TYPE = '"+ type.toString() +"' ORDER BY id");
			...
			if(Main.getMySQL().isConnected()) {
				ResultSet rs = Main.getMySQL().query("SELECT * FROM StaffCore_reasonsdb WHERE TYPE = '"+ type.toString() +"' ORDER BY id");
				
				try {
					while(rs.next()) {
						reasons.add(new Reason(type, rs.getInt("id"), rs.getString("NAME"), rs.getLong("BAN_LENGTH"), rs.getBoolean("ADMIN_BAN")));
					}
					return reasons;
				} catch (SQLException e) {
					e.printStackTrace();
