...
public class TeamChatHandler {
	...
	public void login(ProxiedPlayer player) {
		...
		if(!participants.contains(player)) {
			...
			for(ProxiedPlayer all : participants) {
				all.sendMessage(new TextComponent(prefix + Main.getMSG("Messages.TeamChat.Staff.Joined").replace("%staff%", player.getName())));
			}
	...
	public void logout(ProxiedPlayer player) {
		...
		if(participants.contains(player)) {
			...
			for(ProxiedPlayer all : participants) {
				all.sendMessage(new TextComponent(prefix + Main.getMSG("Messages.TeamChat.Staff.Left").replace("%staff%", player.getName())));
			}
