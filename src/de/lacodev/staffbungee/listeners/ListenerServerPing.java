	...
	public void onPing(ProxyPingEvent e) {
		...
		if(Main.getMaintenanceHandler().isMaintenance()) {
	        ...
	        if(Boolean.valueOf(SettingsManager.getValue(Settings.ADVERTISMENT_ENABLE)).equals(Boolean.TRUE)) {
	        	players.setSample(new ServerPing.PlayerInfo[] {new ServerPing.PlayerInfo("§a", UUID.randomUUID()),new ServerPing.PlayerInfo("§7This Server is operated by §cStaffCore-Bungee", UUID.randomUUID()),new ServerPing.PlayerInfo("§a", UUID.randomUUID())});
	        }
	        ping.setPlayers(players);
	        ping.setVersion(version);
	        ping.setDescriptionComponent(new TextComponent(ChatColor.translateAlternateColorCodes('&', SettingsManager.getValue(Settings.MAINTENANCE_TEXT_LINE1)) + "\n" + ChatColor.translateAlternateColorCodes('&', SettingsManager.getValue(Settings.MAINTENANCE_TEXT_LINE2))));
	        ...
	        ping.setPlayers(players);
	        ping.setVersion(version);
	        ping.setDescriptionComponent(new TextComponent(ChatColor.translateAlternateColorCodes('&', SettingsManager.getValue(Settings.MOTD_TEXT_LINE1)) + "\n" + ChatColor.translateAlternateColorCodes('&', SettingsManager.getValue(Settings.MOTD_TEXT_LINE2))));
		}
	}
