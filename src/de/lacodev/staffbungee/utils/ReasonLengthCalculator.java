...
public class ReasonLengthCalculator {
	...
	public static String calculate(long reasonlength) {
		...
		if(reasonlength != -1) {
			...
			if (tage != 0L) {
				return "§a" + strtage + "§7 " + Main.getMSG("Messages.Layouts.Ban.Remaining.Days");
			}
			if ((tage == 0L) && (stunden != 0L)) {
				return "§a" + strstunden + "§7 " + Main.getMSG("Messages.Layouts.Ban.Remaining.Hours");
			}
			if ((tage == 0L) && (stunden == 0L) && (minuten != 0L)) {
				return "§a" + strminuten + " §7"+ Main.getMSG("Messages.Layouts.Ban.Remaining.Minutes");
			}
